    ...
    for ((i=${#CREATED_RESOURCES[@]}-1; i>=0; i--)); do
        ...
        case "$resource" in
            ...
            group:*)
                ...
                ;;
            subnet:*)
                # Format: subnet:vnet_name:subnet_cidr
...
VLAN_ID=""
SUBNET=""
USERNAME=""  # Will be set to admin_<tenant_name> if not provided
PASSWORD=""
EMAIL=""
DRY_RUN=false
VLAN_PROVIDED=false  # Track if -v flag was used
USERNAME_PROVIDED=false  # Track if -u flag was used
...
if [ "$VLAN_PROVIDED" = false ]; then
    ...
    VLAN_ID=""  # Empty means auto-allocate during VNet creation
else
...
if [ -z "$SUBNET" ]; then
    ...
    SUBNET=""  # Empty means auto-allocate during subnet creation
else
...
if pvesh get "/cluster/sdn/zones/${SDN_ZONE_NAME}" --output-format json &>/dev/null; then
    ...
    ptm_log WARN "You will need to configure networking manually or run tenctl-init.sh first"
fi
